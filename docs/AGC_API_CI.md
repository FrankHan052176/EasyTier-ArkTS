...
The update request refuses to proceed without at least one group. It writes every `groupId`, a start time one hour after the current UTC time, an end time after `AGC_TEST_DURATION_DAYS`, `displayArea="1"`, and `needShareLink=0`. The workflow computes an explicit `AGC_NEED_NOTIFY`: only the first attempt of an ArkTS `push` receives `1`; Core `repository_dispatch` runs, manual runs, and retries receive `0`. The release script refuses `AGC_NEED_NOTIFY=1` outside a first-attempt push and sends that exact integer as `openTestInfo.testTaskInfo.needNotify`, whose AGC-defined `0` value suppresses tester notifications. The Pro workflow keeps this setting at `0`.

AGC has been observed sending a notification for a Core dispatch even when its accepted update payload contained `needNotify=0`. Core dispatches therefore submit against a temporary external group with no testers, while retaining `needNotify=0`. After submission, the workflow calls the official `PUT /api/publish/v2/test/version/open-test` endpoint to bind the configured real groups, then removes the temporary group. Group binding does not call the separate tester-notification endpoint. The resulting invitation-test version remains available to the real groups without exposing any tester to the submission-time notification path. Temporary groups use the `CI silent ` prefix and are excluded from normal group discovery so a failed cleanup cannot turn a later run into a notification target.
