...
Before the App-level `publish/release` build, CI derives its package version from the installed Core HAR. For `2.6.4-52`, it writes `versionName=2.6.4.52`. Its `versionCode` is `<Core version without dots><UTC day of year><daily ohos build number>`; the daily number is calculated from the workflow run sequence and is zero-padded to two digits. This mutation is limited to the runner checkout, so the repository's baseline `AppScope/app.json5` is not changed.
